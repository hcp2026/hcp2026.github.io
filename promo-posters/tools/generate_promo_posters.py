...
def first_paragraph(value: str) -> str:
    value = normalize_text(value) or "待定"
    chunks = [part.strip() for part in re.split(r"\n\s*\n", value) if part.strip()]
    return chunks[0] if chunks else value
...
.topic-panel.combo .topic-speakers {
  grid-template-columns: repeat(5, 1fr);
}
.topic-panel.crypto .topic-speakers {
...
.topic-panel.combo .topic-speaker img {
  height: 100px;
}
.topic-panel.crypto .topic-speaker img {
  height: 110px;
}
.topic-speaker h4 {
  margin: 8px 0 0;
...
def poster_shell(title_cn: str, title_en: str, body: str, extra_css: str) -> str:
...
AGENDA_BLOCKS = [
    ...
    },
    {
        ...
        "place": "密码分析专题",
        "rows": [
            ("14:00-14:30", "Emanuele Bellini", "使用 CLAASP 生成并求解困难的对称密码分析问题", ""),
...
POSTER_AFF_OVERRIDES = {
    "emanuele-bellini": "阿联酋技术创新研究院（TII）"
}
...
def keynotes_html(speakers: dict[str, dict]) -> str:
    ...
    for sid in KEYNOTE_IDS:
        ...
        abstract = POSTER_ABSTRACT_OVERRIDES.get(sid, first_paragraph(pick(s["abstract"])))
  ...
  <div>
    ...
    return poster_shell("特邀报告", "", body, KEYNOTE_CSS)


TOPIC_GROUPS = [
    ("ai", "人工智能专题", "3 位报告人", ["mingxuan-yuan", "zhaoguo-wang", "min-li"]),
    ("combo", "组合优化专题", "5 位报告人", ["yixin-cao", "zhendong-lei", "shengxin-liu", "hu-qin", "yixiao-huang"]),
    ("crypto", "密码分析专题", "4 位报告人", ["emanuele-bellini", "chunning-zhou", "xindi-zhang", "yanhong-fan"]),
]


def topic_card(speakers: dict[str, dict], sid: str) -> str:
    s = speaker(speakers, sid)
    name = pick(s["name"])
    aff = POSTER_AFF_OVERRIDES.get(sid, pick(s["aff"]))
    title = POSTER_TITLE_OVERRIDES.get(sid, pick(s["title"]))
    return f"""<article class="topic-speaker">
  <img src="{photo_path(s)}" alt="{esc(name)}" />
  <h4>{esc(name)}</h4>
  <div class="aff">{esc(aff)}</div>
  <div class="talk">{esc(truncate(title, 54))}</div>
</article>"""


def topics_html(speakers: dict[str, dict]) -> str:
    panels = []
    for cls, title, subtitle, ids in TOPIC_GROUPS:
        cards = "".join(topic_card(speakers, sid) for sid in ids)
        panels.append(
            f"""<section class="topic-panel {cls}">
  <div class="topic-head"><h3>{esc(title)}</h3><div class="note">{esc(subtitle)}</div></div>
  <div class="topic-speakers">{cards}</div>
</section>"""
        )
    body = f'<div class="topic-stack">{"".join(panels)}</div>'
    return poster_shell("专题报告人", "", body, TOPICS_CSS)
...
def clean_outputs() -> None:
    ...
    for directory in [HTML_DIR, PNG_DIR, JPG_DIR, PDF_DIR]:
        for path in directory.iterdir():
            if path.is_file() and path.suffix.lower() in {".html", ".png", ".jpg", ".pdf"}:
                path.unlink()
